...
function addMessage(req, res) {
  ...
  req.on("end", function () {
    ...
    conn.connect(function(err) {
      ...
      // query the database
      conn.query("INSERT INTO MESSAGE (Title, Text) VALUE (?, ?)", [injson.title, injson.text], function(err, rows, fields) {
        // build json result object
        var outjson = {};
        if (err) {
          // query failed
...
function deletePost(req, res) {
  ...
  req.on("end", function () {
    ...
    conn.connect(function(err) {
      ...
      // query the database
      conn.query("DELETE FROM MESSAGE WHERE MessageID=?", [injson.id], function(err, rows, fields) {
        // build json result object
        var outjson = {};
        if (err) {
          // query failed
